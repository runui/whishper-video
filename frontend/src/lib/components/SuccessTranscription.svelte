...
<script>
    ...
    import {deleteTranscription, displayTranscriptionName} from "$lib/utils.js";
    export let tr;

    const dispatch = createEventDispatcher();

    $: translationCount = (tr.translations || []).length +
        (tr.subtitleTracks || []).reduce((sum, t) => sum + (t.translations?.length || 0), 0);
...
<div class="alert alert-success p-3">
    ...
    <span class="flex-1 min-w-0">
        ...
        <p class="font-mono text-info-content text-sm opacity-60 flex space-x-2 md:space-x-4 lg:space-x-8">
            ...
            <span class="space-x-1">
                <span class="font-bold text-xs">{translationCount} translations</span>
            </span>
